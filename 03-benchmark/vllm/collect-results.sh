...
print_stack() {
  ...
  for f in $(ls ./results/${stack}-rate*.json | sort -t e -k3n); do
    local rate ttft50 ttft99 tpot50 tpot99 req_thru out_thru meets
    ...
    printf "  %-8s %-10.2f %-12.1f %-10.1f %-10.1f %-10.2f %-12.2f %s\n" \
      "$rate" "$req_thru" "$out_thru" "$ttft50" "$ttft99" "$tpot50" "$tpot99" "$meets"
  done
